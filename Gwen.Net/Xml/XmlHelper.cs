...
public class XmlHelper {
	...
	public static int[] ParseArrayInt32(string valueStr) {
		...
		foreach(string value in values) {
			if(!int.TryParse(value, NumberStyles.Integer, Parser.NumberFormatInfo, out newValues[index++]))
				throw new Exception($"String \"{value}\" is not a valid integer.");
		}
			...
			{ typeof(Font), (element, value) =>
				{
					...
					string[] fontValues = value.Split(fontValueSeperator);
					...
					if (element is Control.ControlBase control)
						return Font.Create(control.Skin.Renderer, name, size, style);
					else if (element is Component comp && comp.View != null)
						return Font.Create(comp.View.Skin.Renderer, name, size, style);
					else
	...
	private static char[] fontValueSeperator = { ';' };
}