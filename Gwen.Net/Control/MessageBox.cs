...
public class MessageBox : Window {
	private readonly RichLabel text;

	/// <summary>
	/// Invoked when the message box has been dismissed.
	/// </summary>
	public event GwenEventHandler<MessageBoxResultEventArgs>? Dismissed;
	...
	public MessageBox(ControlBase? parent, string text, string caption = "", MessageBoxButtons buttons = MessageBoxButtons.OK)
		...
		this.text = new RichLabel(layout);
		this.text.Dock = Dock.Fill;
		this.text.Margin = Margin.Ten;
		this.text.Document = new Document(text);
	...
	private void DismissedHandler(ControlBase control, EventArgs args) {
		Dismissed?.Invoke(this, new MessageBoxResultEventArgs() { Result = (MessageBoxResult)(control.UserData ?? throw new Exception("Impossible unless user is doing something they shouldn't")) });
	}
}