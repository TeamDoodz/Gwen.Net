...
#pragma warning disable CS8625 // Cannot convert null literal to non-nullable reference type.
		...
		//TODO: Figure out why null is being passed to these methods and fix it
...
#pragma warning restore CS8625 // Cannot convert null literal to non-nullable reference type.
	...
	public static void OnCanvasThink(ControlBase control) {
		...
		for(int i = 0; i < (int)GwenMappedKey.Count; i++) {
			...
			if(keyData.KeyState[i] && time > keyData.NextRepeat[i]) {
				keyData.NextRepeat[i] = Platform.GwenPlatform.GetTimeInSeconds() + KeyRepeatRate;
	...
	public static bool OnMouseClicked(ControlBase canvas, int mouseButton, bool down) {
		...
		if(down &&
			...
			(Platform.GwenPlatform.GetTimeInSeconds() - lastClickTime[mouseButton]) < DoubleClickSpeed) {
			isDoubleClick = true;
		}

		if(down && !isDoubleClick) {
			lastClickTime[mouseButton] = Platform.GwenPlatform.GetTimeInSeconds();
	...
	public static bool OnKeyEvent(ControlBase canvas, GwenMappedKey key, bool down) {
		...
		if(down) {
			if(!keyData.KeyState[iKey]) {
				...
				keyData.NextRepeat[iKey] = Platform.GwenPlatform.GetTimeInSeconds() + KeyRepeatDelay;
